...
CHANNEL_SCRIPT_DIR="${SCRIPT_DIR}/verification_chemistry_baseline"
...
if [ $# -eq 0 ]; then
  SCRIPT_NAME="verification_using_areas.py"
  CONFIG_FILE="confs/config.yaml"
  ARGS="${SCRIPT_NAME} --conf_path ${CONFIG_FILE} --job_id ${SLURM_JOB_ID}"
else
  ARGS="${@}"
fi
